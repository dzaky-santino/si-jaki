...
@section('title', 'Data Histori Perguruan Tinggi')
...
<section class="section">
    <div class="section-header">
        <h1>Data Histori Perguruan Tinggi</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="{{ route('home') }}">Beranda</a></div>
            <div class="breadcrumb-item">Data Histori Perguruan Tinggi</div>
        </div>
    </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header flex-column align-items-start">
                        <!-- Button 'Tambah Data Histori' -->
                        <div class="mb-3 text-center">
                            <a href="{{ route('data-histori-pt.create') }}" class="btn btn-primary centered-button">
                                <i class="fas fa-plus"></i> Tambah Data Histori
                            </a>
                            <a href="#" class="btn btn-primary centered-button" onclick="event.preventDefault(); if(confirm('Apakah Anda yakin ingin menghapus semua data?')) { document.getElementById('delete-all-form').submit(); }">
                         ...
                         <form id="delete-all-form" action="{{ route('data-histori-pt.destroyAll') }}" method="POST" style="display: none;">
                             @csrf
                             @method('DELETE')
                         </form>
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table modern-table">
                                ...
                                <tbody id="dataTable">
                                    @foreach($dataHistoriPT as $key => $data)
                                    <tr class="table-row">
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $data->kode_pt }}</td>
                                        <td>{{ $data->nama_pt }}</td>
                                        <td>
                                            ...
                                            @if($data->status_pt !== 'Aktif' && $data->keterangan)
                                                <a href="#" data-toggle="modal" data-target="#keteranganModal{{ $data->uuid }}" title="Lihat Keterangan">
                                        ...
                                        <td>
                                            ...
                                            <!-- Delete Button -->
                                            <form action="{{ route('data-histori-pt.destroy', $data->uuid) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <div class="modal fade" id="keteranganModal{{ $data->uuid }}" tabindex="-1" role="dialog" aria-labelledby="keteranganModalLabel{{ $data->uuid }}" aria-hidden="true" data-backdrop="false">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="keteranganModalLabel{{ $data->uuid }}">Keterangan</h5>
...
<script>
    ...
    $('[data-toggle="modal"]').on('click', function (e) {
        ...
        $(targetModal).modal({
            backdrop: false, // Nonaktifkan backdrop gelap
            keyboard: true,  // Modal dapat ditutup dengan keyboard
        });
    });
    });
</script>
